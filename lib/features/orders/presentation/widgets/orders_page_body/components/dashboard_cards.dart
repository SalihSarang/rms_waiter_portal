      ...
      builder: (context, state) {
        ...
        if (state is OrdersLoaded) {
          ...
          final completedOrders =
              orders.where((order) => order.orderStatus.name == 'completed').toList();
          servedCount = completedOrders.length.toString();

          final earnings =
              completedOrders.fold<double>(0, (sum, order) => sum + order.totalAmount);
          totalEarnings = '₹${earnings.toStringAsFixed(0)}';

          final pendingOrders =
              orders.where((order) => order.orderStatus.name == 'pending').toList();
